    ...
    const fetchData = async () => {
      try {
        ...
        const [{ data: y1 }, { data: y2 }] = await Promise.all([
          supabase.from('datos_maestros').select('anio').eq('cod_indicador', 'ATAL_01').eq('cod_entidad', '17001').order('anio', { ascending: false }).limit(1),
          supabase.from('datos_maestros').select('anio').eq('cod_indicador', 'ATAL_02').eq('cod_entidad', '17001').order('anio', { ascending: false }).limit(1),
        ]);
