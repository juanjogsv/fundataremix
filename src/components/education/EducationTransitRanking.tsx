    ...
    const loadYears = async () => {
      try {
        const { data: yearsData, error: yearsError } = await supabase
          ...
          .select('year')
          .or('indicador.ilike.%tránsito%,indicador.ilike.%transito%')
          .eq('categoria', 'Total')
      ...
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            <TrendingUp className="h-6 w-6 text-luker-teal" />
            <CardTitle className="text-xl text-luker-brown">
              Ranking de Entidades: Tasa de Tránsito
            </CardTitle>
          </div>
          <div className="flex items-center gap-2">
            <ChartDownloadButton 
              chartRef={chartRef} 
              title={`Ranking Tasa de Tránsito ${selectedYear || ''}`} 
            />
            <Select
            ...
            >
              ...
              >
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.entidad === 'Manizales' ? 'hsl(var(--luker-red))' : 'hsl(var(--luker-teal))'}
                  />
                ))}
              </Bar>
            </BarChart>
...
export default EducationTransitRanking;
