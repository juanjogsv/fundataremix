...
const EducationSecondaryRanking = () => {
    ...
    const loadYears = async () => {
      try {
        const { data: yearsData, error: yearsError } = await supabase
          ...
          .select('year')
          .ilike('indicador', '%cobertura neta%secundaria%')
          .eq('categoria', 'Total')
      ...
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            <TrendingUp className="h-6 w-6 text-luker-green" />
            <CardTitle className="text-xl text-luker-brown">
              Ranking de Entidades: Cobertura Neta en Secundaria
            </CardTitle>
          </div>
          <div className="flex items-center gap-2">
            <ChartDownloadButton 
              chartRef={chartRef} 
              title={`Ranking Cobertura Neta Secundaria ${selectedYear || ''}`} 
            />
            <Select
            ...
            >
              ...
              >
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.entidad === 'Manizales' ? 'hsl(var(--luker-red))' : 'hsl(var(--luker-green))'}
                  />
                ))}
              </Bar>
            </BarChart>
...
export default EducationSecondaryRanking;
