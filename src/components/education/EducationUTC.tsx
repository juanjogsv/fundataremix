...
import EducationUniversityEnrollment from "./EducationUniversityEnrollment";
import EducationUTCPrograms from "./EducationUTCPrograms";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Error fetching UTC data:', err);
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Error al cargar los datos de UTC: {error}
        </AlertDescription>
      </Alert>
    );
  }
        ...
        <Card className="border-luker-green/20 shadow-lg h-full flex flex-col">
          ...
          <CardContent className="pt-6 flex-1 flex flex-col">
            ...
            <div className="flex justify-end pt-4 mt-auto">
              <ChartDownloadButton 
                chartRef={chartRef} 
                title={`Matrícula Técnica en Instituciones Educativas - ${selectedInstitution}`}
                excelData={chartData}
                excelColumns={[
                  { header: "Año", key: "año" },
