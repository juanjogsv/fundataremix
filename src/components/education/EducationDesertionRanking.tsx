...
const EducationDesertionRanking = () => {
      ...
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2">
            <ChartDownloadButton 
              chartRef={chartRef} 
              title={`Ranking Tasa de Deserción ${selectedYear || ''}`} 
            />
            <Select
            ...
            >
              ...
              >
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.entidad === 'Manizales' ? 'hsl(var(--luker-green))' : 'rgb(220, 38, 38)'}
                  />
                ))}
              </Bar>
            </BarChart>
...
export default EducationDesertionRanking;
